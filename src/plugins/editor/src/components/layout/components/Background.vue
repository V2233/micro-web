<template>
  ...
  <template v-if="type === 2">
    ...
    <el-row :gutter="10" v-if="gradientType === 'linear'">
      ...
      <el-col :span="12">
        <el-slider size="small" v-model="gradientRotate" :max="360" />
      </el-col>
    </el-row>
  </template>
</template>
...
const value = computed({
  get: () => {
    return (props.modelValue || '').includes('gradient')
      ? getGradientValue('startColor')
      : props.modelValue
  },
  set: (val) => {
...
function getGradientValue(type: string) {
  ...
  if (!isgradient) {
    // 不是渐变
    return type === 'startColor'
      ? color(gradientString).rgb().string()
      : defaultValueMap[type]
  }

  let gradientRegex =
    /gradient\((\d+deg),\s*(rgba?\([^)]+\)),\s*(rgba?\([^)]+\))\)/
