...
export default class Onebot {
    ...
    constructor(public protocolUrl = '/onebot/v11/ws') {
        ...
        this.url = customAddress?customAddress:`ws://${originUrl.hostname}:${userStore.originPort?userStore.originPort:originUrl.port}${protocolUrl}`
        this.bot = null
        this.msgQueue = []
        this.res = null
        ...
        this.bot.addEventListener('open', (e) => {
            ...
            if(devStore[devStore.curAdapter].settings.heart_beat) {
                this.heartbeat_id = setInterval(()=>{
                    this.bot?.send(JSON.stringify(Events.heartbeat({
